...
const Education = () => {
  return (
    <div id="education" className="mt-20  p-5 scroll-mt-20 w-[90%]">
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 place-items-center justify-center space-y-10 lg:space-y-0 gap-5">
        {educationData.map((item) => {
          return (
            <div
              key={item.id}
              className=" tracking-widest space-y-3 bg-white/7 w-[300px] h-[420px] py-7 px-5 rounded-lg transition-all duration-400 hover:scale-105"
            >
              ...
              <img
                src={item.image}
                alt="image"
                className="w-60 h-40 object-cover mt-10"
              />
            </div>
          );
        })}
